param (
    ...
    #$token = '', #API Token
    $username = 'mcmiv413',
    $scriptrepo='powershell',
    $configrepo='config',
    $productionorg = 'mcmiv413-master',
    $betaorg = 'mcmiv413-beta', 
    $branchcleanday = 'Sunday', #Day of week to run cleanup
    $branchcleanstartutc = '11', #Cleanup window start time in UTC
    $branchcleanendutc = '12' #Cleanup window end time in UTC
)
...
{
    param ( $org )
    ($(invoke-webrequest "https://$username@$githuburl/orgs/$org/repos" -UseBasicParsing).Content | #repos?per_page=200&type=org&sort=full_name&direction=asc&access_token=$token
        ConvertFrom-Json) | 
...
function Update-LocalGits()
{
    ...
    if ($env:dev) { $gits = $gits | Where-Object { $_.fullname -notlike '*\fork\*'}}
    ForEach($git in $gits)
    {
...
if ( $DEV) 
{ 
    [Environment]::SetEnvironmentVariable("DEV", $true, "Machine") 
    $env:DEV = $true
}
...
#$configpath = $($($(Get-ChildItem $psscriptroot\..\..\..\config -File)[0]).directory).fullname
if ( $CurrentConfigValue -notlike "*$configpath*" ) { [Environment]::SetEnvironmentVariable("ConfigPath", $configpath, "Machine") }
$env:PSModulePath = [Environment]::GetEnvironmentVariable("PSModulePath", "Machine")
$env:configpath = [Environment]::GetEnvironmentVariable("ConfigPath", "Machine")